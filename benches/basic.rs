...
fn bench_test(b: &mut Bencher) {
    let server_thread = thread::spawn(move || {
        ...
        let mut server_conn =
            OssuaryConnection::new(ConnectionType::UnauthenticatedServer, Some(auth_secret_key))
                .unwrap();
        while !server_conn.handshake_done().unwrap() {
        ...
        loop {
            ...
            if plaintext == [0xde, 0xde, 0xbe, 0xbe] {
                ...
                if let Ok(dur) = start.elapsed() {
                    let t = dur.as_secs() as f64 + dur.subsec_nanos() as f64 * 1e-9;
    ...
    while !client_conn.handshake_done().unwrap() {
    ...
    let mut bytes: u64 = 0;
    let start = std::time::SystemTime::now();
    let plaintext: &[u8] = &[0xaa; 16384];
    b.iter(
        || match client_conn.send_data(&plaintext, &mut client_stream) {
            Ok(b) => bytes += b as u64,
            Err(OssuaryError::WouldBlock(_)) => {}
            _ => panic!("send error"),
        },
    ...
    let plaintext: &[u8] = &[0xde, 0xde, 0xbe, 0xbe];
    loop {
        match client_conn.send_data(&plaintext, &mut client_stream) {
            Ok(w) => {
