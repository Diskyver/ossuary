...
//
use ossuary::clib::{
    ossuary_add_authorized_key, ossuary_add_authorized_keys, ossuary_create_connection,
    ossuary_destroy_connection, ossuary_handshake_done, ossuary_recv_data, ossuary_recv_handshake,
    ossuary_remote_public_key, ossuary_send_data, ossuary_send_handshake, ossuary_set_secret_key,
...
unsafe fn server() -> Result<(), std::io::Error> {
    ...
    let key: &[u8; 32] = &[
        0xbe, 0x1c, 0xa0, 0x74, 0xf4, 0xa5, 0x8b, 0xbb, 0xd2, 0x62, 0xa7, 0xf9, 0x52, 0x3b, 0x6f,
        0xb0, 0xbb, 0x9e, 0x86, 0x62, 0x28, 0x7c, 0x33, 0x89, 0xa2, 0xe1, 0x63, 0xdc, 0x55, 0xde,
        0x28, 0x1f,
    ];
    ...
    while ossuary_handshake_done(conn) == 0 {
        ...
        let wrote = ossuary_send_handshake(conn, (&out_buf) as *const u8 as *mut u8, &mut out_len);
        if wrote >= 0 {
            ...
            if in_len > 0 {
                let len =
                    ossuary_recv_handshake(conn, in_buf as *const [u8] as *const u8, &mut in_len);
    ...
    plaintext[0..13].copy_from_slice(b"from server 1");
    let mut out_len: u16 = out_buf.len() as u16;
    let sz = ossuary_send_data(
        conn,
        (&plaintext) as *const u8 as *mut u8,
        13 as u16,
    ...
    plaintext[0..13].copy_from_slice(b"from server 2");
    let mut out_len: u16 = out_buf.len() as u16;
    let sz = ossuary_send_data(
        conn,
        (&plaintext) as *const u8 as *mut u8,
    ...
    if !in_buf.is_empty() {
        ...
        if len != -1 {
            ...
            msgs.push(
                std::str::from_utf8(&out_buf[0..out_len as usize])
                    .unwrap()
                    .into(),
            );
            reader.consume(len as usize);
...
unsafe fn client() -> Result<(), std::io::Error> {
    ...
    let mut plaintext: [u8; 256] = [0; 256];
    plaintext[0..11].copy_from_slice(b"from client");
    let mut out_len: u16 = out_buf.len() as u16;
    let sz = ossuary_send_data(
        conn,
        (&plaintext) as *const u8 as *mut u8,
    ...
    assert_eq!(
        msgs,
        vec!("from server 1".to_string(), "from server 2".to_string())
    );
