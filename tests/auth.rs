...
const SERVER_SECRET: &[u8] = &[
    ...
    0xa0, 0x9e, 0xd4, 0xb7, 0x1a, 0xf9, 0xc9, 0x59, 0x3e, 0xa3, 0x1c, 0x85, 0x0f, 0xc4, 0xfa, 0xa2,
];
const SERVER_PUBLIC: &[u8] = &[
    0x20, 0x88, 0x55, 0x8e, 0xbd, 0x9b, 0x46, 0x1d, 0xd0, 0x9d, 0xf0, 0x00, 0xda, 0xf4, 0x0f, 0x87,
    0xf7, 0x38, 0x40, 0xc5, 0x54, 0x18, 0x57, 0x60, 0x74, 0x39, 0x3b, 0xb9, 0x70, 0xe1, 0x46, 0x98,
];
const CLIENT_SECRET: &[u8] = &[
    0x10, 0x86, 0x6e, 0xc4, 0x8a, 0x11, 0xf3, 0xc5, 0x6d, 0x77, 0xa6, 0x4b, 0x2f, 0x54, 0xaa, 0x06,
    0x6c, 0x0c, 0xb4, 0x75, 0xd8, 0xc8, 0x7d, 0x35, 0xb4, 0x91, 0xee, 0xd6, 0xac, 0x0b, 0xde, 0xbc,
];
const CLIENT_PUBLIC: &[u8] = &[
    0xbe, 0x1c, 0xa0, 0x74, 0xf4, 0xa5, 0x8b, 0xbb, 0xd2, 0x62, 0xa7, 0xf9, 0x52, 0x3b, 0x6f, 0xb0,
    0xbb, 0x9e, 0x86, 0x62, 0x28, 0x7c, 0x33, 0x89, 0xa2, 0xe1, 0x63, 0xdc, 0x55, 0xde, 0x28, 0x1f,
];
...
const CLIENT_PUBLIC2: &[u8] = &[
    0xe8, 0x35, 0x9f, 0xd1, 0xc1, 0x99, 0x55, 0x78, 0x4a, 0x0a, 0xea, 0xc2, 0x4b, 0x58, 0x5e, 0x30,
    0xfc, 0x76, 0x0e, 0x84, 0x28, 0xe8, 0x6c, 0x85, 0xb8, 0xe1, 0xa0, 0x47, 0x86, 0x6d, 0xd6, 0xf1,
];
...
fn is_done(
    ...
    match conn.handshake_done() {
        Ok(true) => true,
        Ok(false) => false,
        Err(OssuaryError::UntrustedServer(key)) => match expected_error {
            Some(ExpectedError::UntrustedServer) => match tofu {
                true => {
                    let _ = conn.add_authorized_key(&key).unwrap();
                    false
                }
...
fn auth_client_key_one_good() {
    ...
    let client_conn = OssuaryConnection::new(ConnectionType::Client, Some(CLIENT_SECRET)).unwrap();
    let mut server_conn =
        OssuaryConnection::new(ConnectionType::AuthenticatedServer, None).unwrap();
...
fn auth_server_key_good() {
    ...
    let server_conn =
        OssuaryConnection::new(ConnectionType::UnauthenticatedServer, Some(SERVER_SECRET)).unwrap();
...
fn auth_client_server_keys_good() {
    ...
    let mut client_conn =
        OssuaryConnection::new(ConnectionType::Client, Some(CLIENT_SECRET)).unwrap();
    let mut server_conn =
        OssuaryConnection::new(ConnectionType::AuthenticatedServer, Some(SERVER_SECRET)).unwrap();
    let _ = server_conn.add_authorized_key(CLIENT_PUBLIC).unwrap();
    let _ = client_conn.add_authorized_key(SERVER_PUBLIC).unwrap();
    connect(client_conn, server_conn, None, false);
}
