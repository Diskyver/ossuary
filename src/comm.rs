    ...
    U: std::io::Read,
{
    ...
    if conn.read_buf_used < header_size + packet_len {
        if header_size + packet_len > PACKET_BUF_SIZE {
            return Err(OssuaryError::InvalidPacket("Oversized packet"));
        }
...
impl OssuaryConnection {
        ...
        match self.state {
            ...
            _ => {
                return Err(OssuaryError::InvalidPacket(
                    "Encrypted channel not established.",
                ));
            }
        }
        ...
        V: std::io::Write,
    {
        ...
        let result: Result<usize, OssuaryError> = match pkt.kind() {
            ...
            _ => Err(OssuaryError::InvalidPacket(
                "Received non-encrypted data on encrypted channel.",
            )),
        };
    ...
    where
        ...
        V: std::io::Write,
    {
        write_stored_packet(self, out_buf.deref_mut())
    }
}
