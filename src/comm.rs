    ...
    U: std::io::Read,
{
    ...
    unsafe {
        ...
        std::ptr::copy::<u8>(
            conn.read_buf
                .as_ptr()
                .offset((header_size + packet_len) as isize),
...
/// On success, returns a NetworkPacket struct containing the header and data,
/// and a `usize` indicating how many bytes were consumed from the input buffer.
pub(crate) async fn read_packet_async<T, U>(
    conn: &mut OssuaryConnection,
    mut stream: T,
) -> Result<(NetworkPacket, usize), OssuaryError>
where
...
pub(crate) fn write_stored_packet<T>(
    conn: &mut OssuaryConnection,
    stream: &mut T,
...
///
/// On success, returns the number of bytes written to the output buffer
pub(crate) async fn write_stored_packet_async<T>(
    conn: &mut OssuaryConnection,
    stream: &mut T,
) -> Result<usize, OssuaryError>
where
...
///
/// On success, returns the number of bytes written to the output buffer.
pub(crate) async fn write_packet_async<T>(
    conn: &mut OssuaryConnection,
    stream: &mut T,
    data: &[u8],
    kind: PacketType,
    ...
    T: tokio::io::AsyncWrite + Unpin,
{
    ...
    conn.write_buf_used = 8 + data.len();
    conn.local_msg_id += 1;
    let written = write_stored_packet_async(conn, stream).await?;
    Ok(written)
}
...
impl OssuaryConnection {
    ...
    /// packet.
    pub fn send_data<T, U>(&mut self, in_buf: &[u8], mut out_buf: T) -> Result<usize, OssuaryError>
    where
        T: std::ops::DerefMut<Target = U>,
        U: std::io::Write,
    {
        ...
        buf.extend(&tag);
        let written = write_packet(self, out_buf.deref_mut(), &buf, PacketType::EncryptedData)?;
        Ok(written)
    }
    ...
    /// use [`OssuaryConnection::flush()`] to explicitly finish writing the
    /// packet.
    pub async fn send_data_async<T, U>(
        &mut self,
        in_buf: &[u8],
        mut out_buf: T,
    ) -> Result<usize, OssuaryError>
    ...
    {
        // Try to send any unsent buffered data
        match write_stored_packet_async(self, out_buf.deref_mut()).await {
            Ok(w) if w == 0 => {}
            Ok(w) => return Err(OssuaryError::WouldBlock(w)),
            Err(e) => return Err(e),
        }
        ...
        buf.extend(&tag);
        let written =
            write_packet_async(self, out_buf.deref_mut(), &buf, PacketType::EncryptedData).await?;
        Ok(written)
    }
    ...
    /// error, but indicates that some bytes were read from `in_buf`.  This
    /// indicates that an incomplete packet was received.
    pub async fn recv_data_async<T, U, R, V>(
        &mut self,
        in_buf: T,
        mut out_buf: R,
    ) -> Result<(usize, usize), OssuaryError>
        ...
        V: std::io::Write,
    {
        ...
        let (pkt, bytes) = match read_packet_async(self, in_buf).await {
            Ok(t) => t,
            Err(e @ OssuaryError::WouldBlock(_)) => {
                return Err(e);
            }
    ...
    pub fn flush<R, V>(&mut self, mut out_buf: R) -> Result<usize, OssuaryError>
    where
        R: std::ops::DerefMut<Target = V>,
        V: std::io::Write,
    {
